...
const Gravity = forwardRef(({
    ...
    const initializeRenderer = useCallback(() => {
        ...
        // Use the canvas element created by Render
        const mouse = Mouse.create(render.current.canvas);
        mouse.pixelRatio = window.devicePixelRatio || 1;

        // Allow scrolling over the canvas by removing wheel listeners
        mouse.element.removeEventListener("mousewheel", mouse.mousewheel);
        mouse.element.removeEventListener("DOMMouseScroll", mouse.mousewheel);
        mouse.element.removeEventListener("wheel", mouse.mousewheel);

        // Touch events might also need handling if they block scroll, but Matter mostly blocks touchmove.
        // For now, focusing on scroll (wheel).
        ...
        const walls = [
            Bodies.rectangle(width / 2, height + 50, width, 100, { isStatic: true, render: { visible: debug } }),
            Bodies.rectangle(width + 50, height / 2, 100, height, { isStatic: true, render: { visible: debug } }),
            Bodies.rectangle(-50, height / 2, 100, height, { isStatic: true, render: { visible: debug } }),
        ];

        if (addTopWall) {
            walls.push(Bodies.rectangle(width / 2, -50, width, 100, { isStatic: true, render: { visible: debug } }));
        }

        const checkHover = () => Query.point(engine.current.world.bodies, mouse.position).length > 0;

        if (grabCursor) {
            Events.on(engine.current, "beforeUpdate", () => {
                if (canvas.current) {
                    if (!mouseDown.current) {
                        // We check if mouse is over a body
                        const hover = checkHover();
                        canvas.current.style.cursor = hover ? "grab" : "default";
                    }
                }
            });

            // Listen on the CANVAS element, not the container div, to correspond with Matter's mouse
    ...
    const reset = useCallback(() => {
        ...
        bodiesMap.current.forEach(({ element, body, props }) => {
            ...
            Matter.Body.setAngle(body, props.angle || 0);
        ...
        handleResize();
        startEngine();
    }, [canvasSize, updateElements, handleResize, startEngine]);
